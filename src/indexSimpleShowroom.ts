...
const createPocketWatchModel = (scene: THREE.Scene, objectsMap: ObjectsMap, actionsMap: ActionsMap) => {
  gltfLoader.load('./static/gltf/watch.gltf/vintage_pocket_watch_1k.gltf', (gltf) => {
    ...
    model.castShadow = true
    scene.add(model)
    const descriptionText = `<b>Часы</b><br>В 1845 году к часам на церкви Святого Иоанна в Эксетере, на западе Англии, добавили интересное новшество — еще одну минутную стрелку, которая на четырнадцать минут опережала первую1. По объяснению газеты Trewman’s Exeter Flying Post, это было «большое удобство для общества», так как теперь часы показывали «не только точное время в Эксетере, но и железнодорожное время»2.
...
<br>Власти некоторых городов быстро уловили пользу стандартизации времени по всей стране и соответствующим образом скорректировали свои часы. Кое-кто обижался на столичное своеволие, держась за мысль, что их время — это «правильное время», как с очаровательным местечковым патриотизмом выразился Flying Post. Много лет декан Эксетера упрямо отказывался перевести часы на городском кафедральном соборе.
...
<br>Точное измерение повсеместно принимаемого времени лежит в основе вычислительных сетей и сетей связи. Однако, как было с кораблями, а затем с поездами, самое большое влияние атомные часы, наверное, ока- зали на путешествия.
